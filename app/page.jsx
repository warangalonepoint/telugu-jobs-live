...
export default function HomePage() {
  ...
  const quick = ["Accountant","Driver","Teacher","Sales","Receptionist","Nurse","Software","Data Entry","Security"];
    ...
    <main className="pb-16">
      <section className="card mt-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-semibold">Find jobs fast</h1>
            <p className="text-sm text-white/60">District-filtered deep links. No scraping. No bs.</p>
          </div>
          <Link href="https://www.warangalonestop.in" target="_blank" className="btn">WarangalOneStop.in</Link>
        </div>

        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div>
            <label className="text-xs text-white/60">District</label>
            <select className="input mt-2" value={district} onChange={(e) => setDistrict(e.target.value)}>
          ...
          <div className="md:col-span-2">
            ...
            <div className="mt-2 flex gap-2">
              ...
              />
              <button className="btn" onClick={goSeo}>Search</button>
            </div>
            <p className="mt-2 text-xs text-white/50">Tip: leave blank to browse all district jobs per site.</p>
          </div>
        </div>

        <div className="mt-4 flex flex-wrap gap-2">
          {quick.map((k) => (
            <button key={k} className="btn text-xs" onClick={() => setQ(k)}>{k}</button>
          ))}
        </div>
      </section>
      ...
      <section className="mt-6">
        ...
        <div className="site-grid">
          {links.map((s) => (
            <a key={s.name} href={s.url} target="_blank" className="card hover:scale-[1.01] transition">
      ...
      <section className="mt-10 card">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {quick.slice(0,6).map((k) => {
            ...
            return <Link key={k} href={`/jobs/${dist}/${kw}`} className="btn text-xs">{district} • {k}</Link>;
          })}
        </div>
      </section>
    </main>
